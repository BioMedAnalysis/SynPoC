...
def ssim_val(img1, img2):
    ...
    dr = np.max([img2.max(), img1.max()]) - np.min([img2.min(), img1.min()])

    ssim_score = ssim(np.squeeze(img1), np.squeeze(img2), data_range=dr)
    return ssim_score
...
def sample_and_test(args):
    ...
    load_checkpoint(checkpoint_file, synpoc_gen, 'synpoc_gen', epoch=str(epoch_chosen), device=device)
    ...
    for iteration, (cond_img, hf_img) in enumerate(data_loader):
        cond_img = cond_img.to(device, non_blocking=True)
        real_data = hf_img.to(device, non_blocking=True)

        x_t = torch.randn_like(real_data).to(device)

        fake_sample = sample_from_model(pos_coeff, cond_img, synpoc_gen,
